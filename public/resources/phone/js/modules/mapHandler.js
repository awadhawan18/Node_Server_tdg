...
/**
 * Created by himanshuwolf on 12/10/15.
 *
 * @requires zo, zodom, googleMap
 */
var route_directions = function (map, places) {
    ...
    function calcRoute(map, source, destination, travel_mode ) {
      ...
      directionsService.route(request, function (response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
          var directionsDisplay = new google.maps.DirectionsRenderer(render_options);
...
var init = function() {
  ...
  function CustomMarker(latlng, map, title, args) {
    this.latlng = latlng;
    this.title = title;
    this.args = args;
    this.setMap(map);
  ...
  for(var i = 0; i < markers.length; i+=1 ) {
    if(markers[i].lat && markers[i].lon) {
      ...
      if(markers[i].activity) {
        marker = new TDG.CustomMarker(position, map, markers[i].title, {marker: juniorMarker});
      } else {
        marker = new TDG.CustomMarker(position, map, markers[i].title, {marker: headMarker});
      }
...
TDG.initializeMap = function(el, mapMarker, options, fitBound, routes) {
